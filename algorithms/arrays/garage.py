...
def garage(initial, final):
    """
    There is a parking lot with only one empty spot. Given the initial state of the parking lot and the final state. Each step we are only allowed to move a car out of its place and move it into the empty spot.
    ...
    >>> garage(initial=[1,2,3,0,4], final=[0,3, 2,1,4])


    数组中的元素交换：其中一个元素值为0， 将0位置的目标车辆开进来（将 0位置空车 与 0位置目标车辆交换）
    -
...
"""
thus:
1 2 3 0 4 -- zero = 3, true, car_to_move = final[3] = 1, pos = initial.index(1) = 0, switched [0], [3]
0 2 3 1 4 -- zero = 0, f, initial[1] != final[1], switched 0,1
2 0 3 1 4 -- zero = 1, t, car_to_move = final[1] = 3, pos = initial.index(3) = 2, switched [1], [2]
2 3 0 1 4 -- zero = 2, t, car_to_move = final[2] = 2, pos = initial.index(2) = 0, switched [0], [2]
